...
class VoteVisualizer:
    ...
    def visualize(self, vote_json_path: str, out_path: str | None = None, circle_diameter: int = 36, per_row: int = 10, analysis_json_path: str | None = None) -> Dict:
        # Read JSON
        ...
        for i, (label, list_) in enumerate(headings):
            ...
            try:
                ...
                w = bbox[2]-bbox[0]
                h = bbox[3]-bbox[1]
            draw.text((x - w/2, y_offset), text, fill='#000000', font=font)
        ...
        for col_idx, key in enumerate(['for','against','abstained']):
            ...
            for idx, it in enumerate(items):
                ...
                tx = cx - w/2
                ty = cy - h/2 - 1
                draw.text((tx, ty), st, fill=textcol, font=font)
